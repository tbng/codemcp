...
class TestHotReloadEntry(MCPEndToEndTestCase):
    ...
    async def test_error_handling(self):
        ...
        error_result = await hot_reload_codemcp(
            subtool="ReadFile", path=non_existent_file, chat_id=chat_id
        )

        # Extract text content from error result
        if isinstance(error_result, list) and hasattr(error_result[0], "text"):
            error_message = error_result[0].text
        else:
            error_message = str(error_result)

        # Check that the error contains the file path
        self.assertIn("does_not_exist.txt", error_message)

        # Also verify it mentions it's an error/exception
        self.assertTrue(
            "Error" in error_message
            or "error" in error_message
            or "Exception" in error_message
